...
plugin.init = async (params) => {
	 ...
	 * */
	routeHelpers.setupPageRoute(router, '/spacex', middleware, [(req, res, next) => {
		winston.info(`[plugins/spacex] In middleware. This argument can be either a single middleware or an array of middlewares`);
		setImmediate(next);
	}], (req, res) => {
		winston.info(`[plugins/spacex] Navigated to ${nconf.get('relative_path')}/spacex`);
		res.render('spacex', { uid: req.uid });
	});

	routeHelpers.setupAdminPageRoute(router, '/admin/plugins/spacex', middleware, [], controllers.renderAdminPage);
};
 ...
 *   curl -X GET \
 * 		http://example.org/api/v3/plugins/spacex/test \
...
plugin.addRoutes = async ({ router, middleware, helpers }) => {
	...
	routeHelpers.setupApiRoute(router, 'get', '/spacex/:param1', middlewares, (req, res) => {
